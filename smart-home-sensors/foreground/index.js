...
var device, sensors, led, ledListen;
...
function initValueListeners() {
  var panel = led.find({ name: 'LED Panel' }).get('state').find({ type: 'Control' });
  ...
  $.each($('[id^="v-"]'), function(i, e) {
    ...
    var name = e.attr('id').split('v-')[1].replace('_', ' ');
    ...
    state.on('change:data', function(model) {
      ...
      if (schema) {
        ...
        var color = chroma.bezier(schema).scale().colors(100);
      ...
      var dataString = +Number(data).toFixed(2)+' '+value.get('number.unit');
  ...
  var colorPicker = function() {
    ...
    if (led) {
      ...
      rgbR.on('change:data', function(model) {
        ...
        htmlColorPicker.parent().find('i').css('color', '#'+col);
      });
...
function initRoundSlider() {
  ...
  var roundSlider = function() {
    ...
    if (led) {
      var model = led.find({ name: 'Brightness' })
        .get('state').find({ type: 'Control' });
...
function initPanel() {
  ...
  $('#v-LED_Panel .panel-icons input').click(function() {
    if (this.value === ledListen) {
      ...
      var data = value.get('state').find({ type: 'Report' }).get('data');
      panel.save({
        data: +Number(data).toFixed(2)+' '+value.get('number.unit')
      }, { patch: true });
    }
  });
}
...
function initSwitch() {
  // Find HTML elements and extract some data:
  var btn = led.find({ name: 'Button' }).get('state').find({ type: 'Report' });
  var brightness = led.find({ name: 'Brightness' }).get('state').find({ type: 'Control' });
  var switchBrigtness = $('#switch-brightness').is(':checked');

  // Listen to the physical button presses and display the current state:
  btn.on('change:data', function(model) {
    var isOn = model.get('data') == '1';
    $('#led-btb-onOff').prop('checked', isOn);

    if (switchBrigtness) {
      if (isOn) {
        var value = $('#roundSlider').roundSlider('option', 'value');
      } else {
        var value = 0;
      }
      brightness.save({ data: value.toString() }, { patch: true });
    }
  });

  btn.emit('change:data', btn);

  // Listen to the button clicks (in the wapp):
  $('#switch-brightness').change(function() {
    switchBrigtness = this.checked;
  })
}
...
function showDetails() {
  ...
  $.each($('.info'), function(i, e) {
    $(e).toggle().parent().find('div:first').toggle();
  });
}
